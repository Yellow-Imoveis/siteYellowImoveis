...
const Simulation = ({ propertyInitialValue }) => {
  ...
  return (
    <div className="md:my-16 p-6 text-center md:text-left md:py-12 shadow bg-yellow-500">
        ...
        <div className="flex flex-col items-center xl:flex-row xl:gap-x-16 xl:items-start mb-8">
          <div>
            ...
            <div className="flex flex-col md:flex-row justify-start md:space-x-32 items-center md:mb-16 mb-8">
              <div className="mb-2">
                <label className="block text-black font-bold mb-2 text-lg" htmlFor="valor">
                  Valor do imóvel
                </label>
                <span className="block text-4xl">{Intl.NumberFormat('pt-BR', {style: 'currency', currency: 'BRL'}).format(propertyValue)}</span>
                <input className={`w-full h-2 bg-white appearance-none input-range`} id="valor" type="range" min={50000} max={2500000} step={50000} value={propertyValue} onChange={(e) => setPropertyValue(e.target.value)} />
              </div>
              ...
              <div className="mb-2">
                ...
                <input className={`w-full h-2 bg-white appearance-none input-range`} id="valor" type="range" min={10} max={90} value={entry} step={10} onChange={(e) => setEntry(e.target.value)} />
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-start md:space-x-32 items-center md:mb-16 mb-8 md:ml-24">
              <div className="mb-2">
                ...
                <input className={`w-full h-2 bg-white appearance-none input-range`} id="valor" type="range" min={3} max={12} value={tax} onChange={(e) => setTax(e.target.value)} />
              </div>
              ...
              <div className="mb-2">
                ...
                <input className={`w-full h-2 bg-white appearance-none input-range`} id="valor" type="range" min={5} max={35} value={time} onChange={(e) => setTime(e.target.value)} />
              </div>
            </div>
          ...
          <div className="my-4">
            {result && (
              <>
                <div className="text-center py-8 px-8 md:px-16 md:py-8 w-full h-full bg-gray-50 rounded-xl shadow-[1px_3px_8px_rgba(0,0,0,0.24)]">
                  ...
                  <span className="block text-lg text-black font-bold mb-2">Amortização ({Intl.NumberFormat('pt-BR', {style: 'currency', currency: 'BRL'}).format(result.amortization)} por mês)</span>
                  <span className="block text-md text-black font-bold mb-2">Juros ({Intl.NumberFormat('pt-BR', {style: 'currency', currency: 'BRL'}).format(result.tax_month)} por mês)</span>
                ...
                <div className="text-center mt-4">
                  ...
                  <Link to={`/imoveis?opcao=comprar&preco_maximo=${propertyValue}`} className="btn btn-warning bg-white shadow-[1px_3px_8px_rgba(0,0,0,0.24)]">
                    Ver imóveis
                  </Link>
                </div>
              </> 
